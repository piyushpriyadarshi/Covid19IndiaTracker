...
@Component
public class Tripura implements Runnable {
    ...
    public void run() {
        ...
        State s=stateService.getStateByName("Tripura");
        List<EmailSubscriber> emailList=emailSubscribeService.findEmailSubscriberByState(s);
        if(emailList.size()==0){
            System.out.println("No Data to send email for Tripura");
            return;
        }
        List<StateWiseData> ll=stateWiseDataService.findByStateNameAndDate(mysqlDateString,"Tripura");
        ...
        personalization.addDynamicTemplateData("state", "Tripura");
