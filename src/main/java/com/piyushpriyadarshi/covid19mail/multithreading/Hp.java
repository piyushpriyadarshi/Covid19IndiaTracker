...
public class Hp implements Runnable {
    ...
    public void run() {
        ...
        State s=stateService.getStateByName("Himachal Pradesh");
        List<EmailSubscriber> emailList=emailSubscribeService.findEmailSubscriberByState(s);
        if(emailList.size()==0){
            System.out.println("No email subscribed for Himachal Pradesh");
            return;
        }
        List<StateWiseData> ll=stateWiseDataService.findByStateNameAndDate(mysqlDateString,"Himachal Pradesh");
        if(ll.size()==0){
        ...
        personalization.addDynamicTemplateData("state", "Himachal Pradesh");
