...
@Component
public class Assam  implements Runnable{
    ...
    public void run() {
        ...
        State s=stateService.getStateByName("Assam");
        List<EmailSubscriber> emailList=emailSubscribeService.findEmailSubscriberByState(s);
        if(emailList.size()==0){
            System.out.println("No email subscribed for Assam");
            return;
        }
        List<StateWiseData> ll=stateWiseDataService.findByStateNameAndDate(mysqlDateString,"Assam");
        if(ll.size()==0){
        ...
        personalization.addDynamicTemplateData("state", "Assam");
