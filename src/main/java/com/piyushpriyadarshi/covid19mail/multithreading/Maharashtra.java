...
@Component
public class Maharashtra implements Runnable {
    @Autowired
    ...
    public void run() {
        ...
        State s=stateService.getStateByName("Maharashtra");
        List<EmailSubscriber> emailList=emailSubscribeService.findEmailSubscriberByState(s);
        if(emailList.size()==0){
            System.out.println("No email subscribed for Maharashtra");
            return;
        }
        List<StateWiseData> ll=stateWiseDataService.findByStateNameAndDate(mysqlDateString,"Maharashtra");
        if(ll.size()==0){
        ...
        personalization.addDynamicTemplateData("state", "Maharashtra");
