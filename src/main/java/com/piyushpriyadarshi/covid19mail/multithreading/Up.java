...
public class Up implements Runnable{
    ...
    public void run() {
        ...
        State s=stateService.getStateByName("Uttar Pradesh");
        List<EmailSubscriber> emailList=emailSubscribeService.findEmailSubscriberByState(s);
        if(emailList.size()==0){
            System.out.println("No email subscribed for Uttar Pradesh");
            return;
        }
        List<StateWiseData> ll=stateWiseDataService.findByStateNameAndDate(mysqlDateString,"Uttar Pradesh");
        if(ll.size()==0){
        ...
        personalization.addDynamicTemplateData("state", "Uttar Pradesh");
