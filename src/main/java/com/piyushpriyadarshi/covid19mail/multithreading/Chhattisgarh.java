...
@Component
public class Chhattisgarh implements Runnable{
    @Autowired
    ...
    public void run() {
        ...
        State s=stateService.getStateByName("Chhattisgarh");
        List<EmailSubscriber> emailList=emailSubscribeService.findEmailSubscriberByState(s);
        if(emailList.size()==0){
            System.out.println("No email subscribed for Chhattisgarh");
            return;
        }
        List<StateWiseData> ll=stateWiseDataService.findByStateNameAndDate(mysqlDateString,"Chhattisgarh");
        if(ll.size()==0){
        ...
        personalization.addDynamicTemplateData("state", "Chhattisgarh");
