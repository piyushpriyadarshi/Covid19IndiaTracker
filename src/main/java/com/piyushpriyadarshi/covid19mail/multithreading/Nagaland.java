...
@Component
public class Nagaland implements Runnable {
    ...
    public void run() {
        ...
        State s=stateService.getStateByName("Nagaland");
        List<EmailSubscriber> emailList=emailSubscribeService.findEmailSubscriberByState(s);
        if(emailList.size()==0){
            System.out.println("No Data to send email for Nagaland");
            return;
        }
        List<StateWiseData> ll=stateWiseDataService.findByStateNameAndDate(mysqlDateString,"Nagaland");
        ...
        personalization.addDynamicTemplateData("state", "Nagaland");
