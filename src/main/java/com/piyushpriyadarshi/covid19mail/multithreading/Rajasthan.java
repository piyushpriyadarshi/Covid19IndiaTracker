...
@Component
public class Rajasthan implements Runnable {
    @Autowired
    ...
    public void run() {
        ...
        State s=stateService.getStateByName("Rajasthan");
        List<EmailSubscriber> emailList=emailSubscribeService.findEmailSubscriberByState(s);
        if(emailList.size()==0){
            System.out.println("No email subscribed for Rajasthan");
            return;
        }
        List<StateWiseData> ll=stateWiseDataService.findByStateNameAndDate(mysqlDateString,"Rajasthan");
        if(ll.size()==0){
        ...
        personalization.addDynamicTemplateData("state", "Rajasthan");
