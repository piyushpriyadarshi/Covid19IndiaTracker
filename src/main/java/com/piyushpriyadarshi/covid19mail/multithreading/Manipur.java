...
@Component
public class Manipur implements Runnable{
    @Autowired
    ...
    public void run() {
        ...
        State s=stateService.getStateByName("Manipur");
        List<EmailSubscriber> emailList=emailSubscribeService.findEmailSubscriberByState(s);
        if(emailList.size()==0){
            System.out.println("No email subscribed for Manipur");
            return;
        }
        List<StateWiseData> ll=stateWiseDataService.findByStateNameAndDate(mysqlDateString,"Manipur");
        if(ll.size()==0){
        ...
        personalization.addDynamicTemplateData("state", "Manipur");
