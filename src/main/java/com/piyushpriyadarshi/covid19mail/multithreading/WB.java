...
public class WB implements Runnable {
    ...
    public void run() {
        ...
        State s=stateService.getStateByName("West Bengal");
        List<EmailSubscriber> emailList=emailSubscribeService.findEmailSubscriberByState(s);
        if(emailList.size()==0){
            System.out.println("No email subscribed for West Bengal");
            return;
        }
        List<StateWiseData> ll=stateWiseDataService.findByStateNameAndDate(mysqlDateString,"West Bengal");
        if(ll.size()==0){
        ...
        personalization.addDynamicTemplateData("state", "West Bengal");
