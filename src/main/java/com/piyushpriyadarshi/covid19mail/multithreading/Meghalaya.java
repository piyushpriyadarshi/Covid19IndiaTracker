...
@Component
public class Meghalaya implements Runnable {
    ...
    public void run() {
        ...
        State s=stateService.getStateByName("Meghalaya");
        List<EmailSubscriber> emailList=emailSubscribeService.findEmailSubscriberByState(s);
        if(emailList.size()==0){
            System.out.println("No Data to send email for Meghalaya");
            return;
        }
        List<StateWiseData> ll=stateWiseDataService.findByStateNameAndDate(mysqlDateString,"Meghalaya");
        ...
        personalization.addDynamicTemplateData("state", "Meghalaya");
