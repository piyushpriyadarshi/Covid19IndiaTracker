...
public class Goa implements Runnable{
    ...
    public void run() {
        ...
        State s=stateService.getStateByName("Goa");
        List<EmailSubscriber> emailList=emailSubscribeService.findEmailSubscriberByState(s);
        if(emailList.size()==0){
            System.out.println("No email subscribed for Goa");
            return;
        }
        List<StateWiseData> ll=stateWiseDataService.findByStateNameAndDate(mysqlDateString,"Goa");
        if(ll.size()==0){
        ...
        personalization.addDynamicTemplateData("state", "Goa");
