...
@Component
public class Haryana implements Runnable{
    @Autowired
    ...
    public void run() {
        ...
        State s=stateService.getStateByName("Haryana");
        List<EmailSubscriber> emailList=emailSubscribeService.findEmailSubscriberByState(s);
        if(emailList.size()==0){
            System.out.println("No email subscribed for Haryana");
            return;
        }
        List<StateWiseData> ll=stateWiseDataService.findByStateNameAndDate(mysqlDateString,"Haryana");
        if(ll.size()==0){
        ...
        personalization.addDynamicTemplateData("state", "Haryana");
