...
@Component
public class Uttarakhand implements Runnable {
    ...
    public void run() {
        ...
        State s=stateService.getStateByName("Uttarakhand");
        List<EmailSubscriber> emailList=emailSubscribeService.findEmailSubscriberByState(s);
        if(emailList.size()==0){
            System.out.println("No Data to send email for Uttarakhand");
            return;
        }
        List<StateWiseData> ll=stateWiseDataService.findByStateNameAndDate(mysqlDateString,"Uttarakhand");
        ...
        personalization.addDynamicTemplateData("state", "Uttarakhand");
