...
@Component
public class Jandk implements Runnable {
    @Autowired
    ...
    public void run() {
        ...
        State s=stateService.getStateByName("Jammu and Kashmir");
        List<EmailSubscriber> emailList=emailSubscribeService.findEmailSubscriberByState(s);
        if(emailList.size()==0){
            System.out.println("No email subscribed for Jammu and Kashmir");
            return;
        }
        List<StateWiseData> ll=stateWiseDataService.findByStateNameAndDate(mysqlDateString,"Jammu and Kashmir");
        if(ll.size()==0){
        ...
        personalization.addDynamicTemplateData("state", "Jammu and Kashmir");
