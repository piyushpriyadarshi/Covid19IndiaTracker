...
@Component
public class Punjab implements Runnable{
    @Autowired
    ...
    public void run() {
        ...
        State s=stateService.getStateByName("Punjab");
        List<EmailSubscriber> emailList=emailSubscribeService.findEmailSubscriberByState(s);
        if(emailList.size()==0){
            System.out.println("No email subscribed for Punjab");
            return;
        }
        List<StateWiseData> ll=stateWiseDataService.findByStateNameAndDate(mysqlDateString,"Punjab");
        if(ll.size()==0){
        ...
        personalization.addDynamicTemplateData("state", "Punjab");
