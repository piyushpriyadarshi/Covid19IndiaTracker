...
@Component
public class Jharkhand implements Runnable {
    @Autowired
    ...
    public void run() {
        ...
        State s=stateService.getStateByName("Jharkhand");
        List<EmailSubscriber> emailList=emailSubscribeService.findEmailSubscriberByState(s);
        if(emailList.size()==0){
            System.out.println("No email subscribed for Jharkhand");
            return;
        }
        List<StateWiseData> ll=stateWiseDataService.findByStateNameAndDate(mysqlDateString,"Jharkhand");
        if(ll.size()==0){
        ...
        personalization.addDynamicTemplateData("state", "Jharkhand");
