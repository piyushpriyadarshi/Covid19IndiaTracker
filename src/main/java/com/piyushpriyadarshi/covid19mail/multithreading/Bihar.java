...
@Component
public class Bihar implements Runnable{
    ...
    public void run() {
        ...
        State s=stateService.getStateByName("Bihar");
        List<EmailSubscriber> emailList=emailSubscribeService.findEmailSubscriberByState(s);
        if(emailList.size()==0){
            System.out.println("No email subscribed for Bihar");
            return;
        }
        List<StateWiseData> ll=stateWiseDataService.findByStateNameAndDate(mysqlDateString,"Bihar");
        if(ll.size()==0){
        ...
        personalization.addDynamicTemplateData("state", "Bihar");
