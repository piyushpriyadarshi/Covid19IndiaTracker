...
@Component
public class Gujarat implements Runnable {
    @Autowired
    ...
    public void run() {
        ...
        State s=stateService.getStateByName("Gujarat");
        List<EmailSubscriber> emailList=emailSubscribeService.findEmailSubscriberByState(s);
        if(emailList.size()==0){
            System.out.println("No email subscribed for Gujarat");
            return;
        }
        List<StateWiseData> ll=stateWiseDataService.findByStateNameAndDate(mysqlDateString,"Gujarat");
        if(ll.size()==0){
        ...
        personalization.addDynamicTemplateData("state", "Gujarat");
