...
@Component
public class Arunachal implements Runnable{
    ...
    public void run() {
        ...
        State s=stateService.getStateByName("Arunachal Pradesh");
        List<EmailSubscriber> emailList=emailSubscribeService.findEmailSubscriberByState(s);
        if(emailList.size()==0){
            System.out.println("No email subscribed for Arunachal pradesh");
            return;
        }
        List<StateWiseData> ll=stateWiseDataService.findByStateNameAndDate(mysqlDateString,"Arunachal Pradesh");
        if(ll.size()==0){
        ...
        personalization.addDynamicTemplateData("state", "Arunachal Pradesh");
