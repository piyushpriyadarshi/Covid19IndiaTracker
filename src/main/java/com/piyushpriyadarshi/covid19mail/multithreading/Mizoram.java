...
@Component
public class Mizoram implements Runnable {
    @Autowired
    ...
    public void run() {
        ...
        State s=stateService.getStateByName("Mizoram");
        List<EmailSubscriber> emailList=emailSubscribeService.findEmailSubscriberByState(s);
        if(emailList.size()==0){
            System.out.println("No email subscribed for Mizoram");
            return;
        }
        List<StateWiseData> ll=stateWiseDataService.findByStateNameAndDate(mysqlDateString,"Mizoram");
        if(ll.size()==0){
        ...
        personalization.addDynamicTemplateData("state", "Mizoram");
